...
function fetchRevisions(adapter) {
  var directory = path.join(adapter.config.remoteDir, 'revisions');
...
function activateRevision(adapter, sftp, revision) {
  ...
  var index = path.join(directory, 'index.html'),
    revisionIndex = path.join(directory, 'revisions', revision, 'index.html');
...
module.exports = SSHAdapter.extend({
  ...
  upload: function(buffer) {
    ...
    return this.connect().then(function() {
      var directory = path.join(adapter.config.remoteDir, 'revisions', metadata.revision);
